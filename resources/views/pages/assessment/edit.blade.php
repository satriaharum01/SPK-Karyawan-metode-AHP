    ...
    <div class="container-fluid py-2">
        ...
        <div class="row" id="hasil_container">
            <div class="card col-12 p-4">
                <div class="row">
                    <div class="col-6">
                        <h4 class="mb-4" style="text-transform: uppercase;">Input Nilai - {{$employee->name}}</h4>
                    </div>
                ...
                <form id="form" method="post" action="{{ route('assessment.update', ['employeeId' => $employee->id]) }}">
                    @csrf
                    @method('POST')
                            ...
                            <table class="table">
                                <thead>
                                    <tr class="text-center">
                                        <th colspan="2" style="text-transform: uppercase;">{{$assessment->criteria}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($assessment->values as $value)
                                    <tr class="text-left">
                                        <td class="fw-bold mx-4" style="width:70%; text-transform: capitalize;">{{$value->name}}</td>
                                        <td style="width:30%">
                                            <select name="{{$value->id}}" class="form-select px-4" style="padding: 5px 20px; width: 100%;">
                                                @foreach ($grade_dropdown as $grade)
                                                <option value="{{$grade->value}}" @if($value->value == $grade->value) selected @endif>{{$grade->label}}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                    </tr>
